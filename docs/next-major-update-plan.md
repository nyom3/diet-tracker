...
| M | error_message_redacted | OpenAIの構造化`error.message`をキー伏字化・最大200字にした値。JSONでない本文やmessage欠落時は空 |
| N | diagnostics | コーチJSONパース失敗時の形状のみ（`finish_reason` / `text_len` / `parts` / `head_brace` / `tail_brace`） |

応答本文、APIキー、キー断片、食事データ、プロンプトは保存しない。`error_message_redacted` はOpenAIの構造化`error.message`だけを入力とし、`sk-` / `AIza`形式を伏字化してから200字に切り詰める。GeminiのエラーメッセージはURLクエリにキーを含みうるため保存しない。`diagnostics` は応答本文を含めず、コーチJSONのパース失敗時だけ形状を記録する。データ行は最大2,000行とし、超過時は古い行から削除する。シート生成・ヘッダー保証・行追加・上限整理・診断値組み立ての失敗はAI本体の成否へ影響させず、best-effortで破棄する。
