...
test('acceptCoachAction/setCoachActionStatusは専用RPCを呼び、CoachActionを正規化する', async () => {
  ...
  const runner = {
    ...
    acceptCoachAction(payload) {
      calls.push(['acceptCoachAction', payload]);
      this.successHandler({ id: 'action_1', key: 'protein', category: 'protein', text: '行動', target_date: '2026-07-16', created_at: '2026-07-15T00:00:00Z', status: 'planned' });
    },
    setCoachActionStatus(id, status) {
      calls.push(['setCoachActionStatus', id, status]);
      this.successHandler({ id, key: 'protein', category: 'protein', text: '行動', target_date: '2026-07-16', created_at: '2026-07-15T00:00:00Z', status, completed_at: status === 'completed' ? '2026-07-15T01:00:00Z' : null });
    },
  };

  globalThis.window = { google: { script: { run: runner } } };
  try {
    const accepted = await gasClient.acceptCoachAction({ scope: 'trend', range_days: 30, action_key: 'protein' });
    const completed = await gasClient.setCoachActionStatus('action_1', 'completed');
    assert.equal(accepted.id, 'action_1');
    assert.equal(completed.status, 'completed');
    assert.deepEqual(calls, [
      ['acceptCoachAction', { scope: 'trend', range_days: 30, action_key: 'protein' }],
      ['setCoachActionStatus', 'action_1', 'completed'],
    ]);
  } finally {
    globalThis.window = previousWindow;
  }
