...
const source = await readFile(new URL('../gas/DashboardMetrics.js', import.meta.url), 'utf8');
const dashboardHtml = await readFile(new URL('../gas/index.html', import.meta.url), 'utf8');
const moduleObject = { exports: {} };
vm.runInNewContext(source, { module: moduleObject });
const metrics = moduleObject.exports;
...
const healthHeaders = ['date', 'steps', 'total_calories_kcal', 'weight_kg', 'body_fat_pct'];

test('DashboardMetrics.jsはGAS互換の単一モジュールで応答サイズを抑える', () => {
  ...
  assert.ok(Buffer.byteLength(dashboardHtml, 'utf8') < 300_000);
});

test('getDashboardPeriod: JST日付境界と7/30/90日をDST非依存で生成する', () => {
  assert.deepEqual(plain(metrics.getDashboardPeriod(7, '2026-03-08T14:59:59Z')), {
    window_start: '2026-03-02',
    window_end: '2026-03-08',
  });
  assert.deepEqual(plain(metrics.getDashboardPeriod(30, '2026-07-15T14:59:59Z')), {
    window_start: '2026-06-16',
    window_end: '2026-07-15',
  });
  assert.deepEqual(plain(metrics.getDashboardPeriod(90, '2026-07-15')), {
...
test('buildDashboardData: 複数食、coverage、未来食、JST境界を集計する', () => {
  ...
  assert.deepEqual(plain(day.coverage), {
...
test('buildDashboardData: health_data同日重複は下側の非空値を項目単位で優先する', () => {
  ...
  assert.deepEqual(plain(result.goals), {
    calories_kcal: 1800,
    protein_g: 120,
    fat_g: null,
    carbs_g: null,
...
test('buildDashboardData: health_dataは列順ではなくヘッダー名で値を読む', () => {
  const result = metrics.buildDashboardData({
    rangeDays: 7,
    now: '2026-07-15',
    foodLogs: [],
    healthHeaders: ['date', 'weight_kg', 'steps', 'body_fat_pct', 'total_calories_kcal'],
    healthRows: [['2026-07-14', 71, 1500, 21, 2100]],
    targets: [],
  });
  const day = result.days.find(({ date }) => date === '2026-07-14');
  assert.deepEqual(day && {
...
test('DashboardMetrics: 10000 food + 5000 health rowsを純粋集計する', () => {
  ...
  const responseSize = Buffer.byteLength(JSON.stringify(metrics.buildDashboardData({
    rangeDays: 90,
    now: '2026-07-15',
    foodLogs,
    healthRows,
  ...
  assert.ok(responseSize < 150_000, `応答が${responseSize} bytesあります`);
});
