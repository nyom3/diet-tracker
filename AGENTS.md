...
## PR 作成・push 後に必ず実行すること

1. PR 作成直後に以下を実行する:
   ```
   echo "https://github.com/nyom3/diet-tracker/pull/<N>" > ../.review-inbox
   ```

2. GitHub コメントを 5 分おき・最大 8 回確認する:
   ```
   gh pr view <PR_URL> --json comments \
     --jq '[.comments[].body | select(contains("claude-review"))] | last'
   ```

3. `claude-review-lgtm` を検知 → タスク完了を報告して終了

4. `claude-review-pending` を検知 → 修正 → push → 手順 1 へ戻る
